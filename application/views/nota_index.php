	...
	<div class="row">
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<select id="id_certificacao" name="id_certificacao" class="form-control">
						<option value=""> - </option>
						<?php foreach ($certificacao as $item): ?>
		...
		<script>
				...
				jQuery('#id_disciplina').change(function(){
					...
					jQuery.ajax({
						type: 'POST',
						url: '<?php echo base_url(); ?>index.php/nota/getAluno',
						data: {id_turma:id_turma}
					}).done(function(response){
						jQuery('#detail').html('<hr>');
						var alunos = JSON.parse(response);
						for(i in alunos)
						{
							var aluno = alunos[i];
							jQuery('#detail').append('<label class="col-sm-7 control-label">'+aluno.nm_aluno+'</label><div class="col-sm-5"><input type="text" name="nota['+ aluno.id +']" class="form-control"></div>');
						}
					});
